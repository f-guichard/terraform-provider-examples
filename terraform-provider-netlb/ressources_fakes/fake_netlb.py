...
PORT = 30001 #Tu es dans ton container, back to back service
CONTROLLER_VERSION = "v1"
_CONTROLLER_NAME = "Netlb Controller"
_VIPS_URL = "/"+CONTROLLER_VERSION+"/network-lb"
_VIP_ID = "/"+CONTROLLER_VERSION+"/network-lb/<netlbid>"
...
def list_flow(netlbid):
    #Ouchhhhhhhhhh
    response = jsonify(ramDic.get(netlbid))
    response.status_code = 200
    return response
...
def create_flows():
    ...
    response = jsonify({'netlbid':str(len(ramDic)-1)},{"netlb":ramDic.get(str(len(ramDic)-1))})
...
def delete_flows(netlbid):
    #Ouchhhhhhhhhh
    response = jsonify(ramDic.pop(netlbid))
    response.status_code = 200
    return response
